...
function FaqItem({ question, answer, accentColor, borderColor }: { question: string; answer: string; accentColor: string; borderColor: string }) {
  ...
  return (
    <div style={{ borderBottom: `1px solid ${borderColor}` }}>
      ...
      >
        <span style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "16px", fontWeight: 700, color: "#1c1610", lineHeight: 1.4 }}>
          {question}
        </span>
        <span style={{ fontFamily: "'Lora', Georgia, serif", fontSize: "20px", color: accentColor, flexShrink: 0, lineHeight: 1 }}>
          {open ? "−" : "+"}
        </span>
      </button>
      {open && (
        <div style={{ fontFamily: "'Lora', Georgia, serif", fontSize: "15px", color: "#6b5a3e", lineHeight: 1.85, paddingBottom: "18px", paddingRight: "24px" }}>
          {answer}
        </div>
      )}
    </div>
...
export default function ArticlePage() {
  ...
  useEffect(() => {
    ...
    document.title = article.title + ' | Best Leather Totes';
    ...
    const pageUrl = 'https://bestwomensleathertotes.com/articles/' + article.slug;
    const catUrl = 'https://bestwomensleathertotes.com/' + article.categorySlug;
    ...
    articleScript.textContent = JSON.stringify({
      ...
      "author": { "@type": "Organization", "name": 'Best Leather Totes' },
      "publisher": { "@type": "Organization", "name": 'Best Leather Totes', "url": 'https://bestwomensleathertotes.com' },
      "url": pageUrl,
    });
    ...
    return (
      <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          <p style={{ color: C.tertiary, fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", marginBottom: "16px" }}>Article not found</p>
          <Link href="/"><span style={{ color: C.accent, fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "13px", cursor: "pointer" }}>← Back to home</span></Link>
        </div>
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.body, overflowX: "hidden", maxWidth: "100vw" }}>
          ...
          <nav style={{ display: "flex", alignItems: "center", gap: "0" }} className="hidden-mobile">
            {topicSections.map((sec) => (
              <Link key={sec.slug} href={`/${sec.slug}`}>
                <span style={{ fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "13px", color: sec.slug === article.categorySlug ? "#faf7f2" : C.faint, padding: "8px 16px", display: "inline-block", cursor: "pointer", borderBottom: sec.slug === article.categorySlug ? `2px solid ${C.accent}` : "2px solid transparent", transition: "color 0.15s, border-color 0.15s" }}
                  onMouseEnter={(e) => { e.currentTarget.style.color = "#faf7f2"; e.currentTarget.style.borderBottomColor = C.accent; }}
                  onMouseLeave={(e) => { e.currentTarget.style.color = sec.slug === article.categorySlug ? "#faf7f2" : C.faint; e.currentTarget.style.borderBottomColor = sec.slug === article.categorySlug ? C.accent : "transparent"; }}
                >
                  {sec.label}
                </span>
              </Link>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 clamp(20px, 5vw, 60px)", display: "flex", gap: "56px", alignItems: "flex-start" }}>
        ...
        <article style={{ flex: 1, minWidth: 0, padding: "48px 0 96px" }}>
          ...
          {/* Breadcrumb */}
          <div style={{ display: "flex", alignItems: "center", gap: "6px", fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "12px", color: C.tertiary, marginBottom: "28px", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "16px", flexWrap: "wrap" }}>
            <span style={{ fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "12px", color: C.accent, letterSpacing: "0.04em" }}>
              {article.category}
            </span>
            {article.articleType && article.articleType !== 'blog' && (
              <span style={{ fontFamily: "'Lora', Georgia, serif", fontSize: "9px", fontWeight: 700, color: C.btnText, background: C.accent, letterSpacing: "0.08em", textTransform: "uppercase", padding: "3px 9px", borderRadius: "1px" }}>
          ...
          {(() => {
            ...
            return (
              <div style={{ display: "flex", alignItems: "center", gap: "16px", fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "12px", color: C.tertiary, marginBottom: "32px", paddingBottom: "28px", borderBottom: `2px solid ${C.border}`, flexWrap: "wrap" }}>
          ...
          {/* Hero image */}
          {article.thumbnail && (
            <div className="thumb-hero" style={{ marginBottom: "36px", borderRadius: "2px", overflow: "hidden", border: `1px solid ${C.border}` }}>
          ...
          {article.articleType === 'comparison' && (() => {
            ...
            return verdictHtml ? (
              <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderLeft: `4px solid ${C.accent}`, borderRadius: "2px", padding: "22px 28px", marginBottom: "36px" }}>
                <div style={{ fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "11px", color: C.accent, letterSpacing: "0.06em", marginBottom: "14px" }}>Quick Verdict</div>
          ...
          {article.articleType === 'listicle' && (() => {
            ...
            return (
              <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderLeft: `4px solid ${C.accent}`, borderRadius: "2px", padding: "22px 28px", marginBottom: "36px" }}>
                <div style={{ fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "11px", color: C.accent, letterSpacing: "0.06em", marginBottom: "14px" }}>In This List</div>
                <ol style={{ margin: 0, padding: "0 0 0 18px", display: "flex", flexDirection: "column", gap: "7px" }}>
                  {items.map((item, i) => {
                    ...
                    return (
                      <li key={i} style={{ fontFamily: "'Lora', Georgia, serif", fontSize: "14px", color: C.secondary, lineHeight: 1.6 }}>
          ...
          {nextArticle && (
            <div style={{ marginTop: "64px", paddingTop: "36px", borderTop: `2px solid ${C.border}` }}>
              <div style={{ fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "11px", color: C.tertiary, letterSpacing: "0.06em", marginBottom: "18px" }}>
                Next in {article.category}
              </div>
              <Link href={`/articles/${nextArticle.slug}`}>
                <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: "2px", padding: "24px", cursor: "pointer", transition: "background 0.15s, box-shadow 0.15s" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.background = C.bgHover; (e.currentTarget as HTMLDivElement).style.boxShadow = "0 4px 16px rgba(44,36,22,0.08)"; }}
                ...
                >
                  ...
                  <p style={{ fontFamily: "'Lora', Georgia, serif", fontSize: "14px", color: C.secondary, lineHeight: 1.75, marginBottom: "14px" }}>{nextArticle.excerpt}</p>
                  <span style={{ fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "13px", color: C.accent, display: "inline-flex", alignItems: "center", gap: "5px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
                {furtherReading.map((fr) => (
                  <Link key={fr.slug} href={`/articles/${fr.slug}`}>
                    ...
                    >
                      <div style={{ fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "11px", color: C.accent, letterSpacing: "0.04em", marginBottom: "5px" }}>
                        {fr.category}
                      </div>
        ...
        {headings.length > 2 && (
          <aside style={{ width: "230px", flexShrink: 0, position: "sticky", top: "80px", paddingTop: "48px" }} className="toc-sidebar">
            <div style={{ fontFamily: "'Lora', Georgia, serif", fontStyle: "italic", fontSize: "11px", color: C.tertiary, letterSpacing: "0.08em", marginBottom: "14px" }}>
              Contents
            </div>
            <nav>
              {headings.map((h) => (
                <a key={h.id} href={`#${h.id}`} style={{ display: "block", fontFamily: "'Lora', Georgia, serif", fontSize: "13px", color: activeHeading === h.id ? C.accent : C.secondary, textDecoration: "none", padding: "6px 0 6px 12px", borderLeft: `2px solid ${activeHeading === h.id ? C.accent : C.borderLight}`, marginBottom: "2px", transition: "color 0.15s, border-color 0.15s", lineHeight: 1.5 }}>
                  {h.text}
                </a>
              ))}
            </nav>
